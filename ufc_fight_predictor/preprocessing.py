...
class DataProcessor:
    def __init__(self, username: str, password: str, protocol: str = "postgresql", server: str = "localhost", port: int = 5432, dbname: str = "ufc"):
        self.db_engine = self.get_db_engine(username, password, protocol, server, port, dbname)
        self.logger = logging.getLogger(__name__)
    
    def get_db_engine(self, username: str, password: str, protocol: str = "postgresql", server: str = "localhost", port: int = 5432, dbname: str = "ufc") -> Engine:
    ...
    def load_fighters(self):
        with self.db_engine.connect() as conn:
            self.logger.info("Loading data from the database...")
            fighters = pd.read_sql('SELECT * FROM ufc.fighters_cleaned order by ufc_wins desc', con=conn)
            self.logger.info("Data loaded successfully.")
        return fighters
    ...
    def preprocess_training_data(self, bouts: pd.DataFrame, fighters: pd.DataFrame):
        ...
        duplicated_fighters = fighters[fighters.duplicated(subset="fighter_name", keep=False)]
        ...
        fighters = fighters.loc[~((fighters["slpm"] == 0) &
        ...
        fighters.to_sql('fighters_cleaned', self.db_engine, schema='ufc', if_exists='replace', index=False)
        ...
        bouts.to_sql('model_input', self.db_engine, schema='ufc', if_exists='replace', index=False)
        self.logger.info("Data preprocessing complete. Model input saved to the database.")

    def calculate_differences_on_inference(self, fighter1:str, fighter2:str) -> pd.DataFrame:
        # Extract fighter statistics for fighter1 and fighter2 from table fighters_cleaned
        fighter1_stats = pd.read_sql(f"SELECT * FROM ufc.fighters_cleaned WHERE fighter_name='{fighter1}'", con=self.db_engine)
        fighter2_stats = pd.read_sql(f"SELECT * FROM ufc.fighters_cleaned WHERE fighter_name='{fighter2}'", con=self.db_engine)

        # Create empty output dataframe
        output_df = pd.DataFrame()

        # Convert all columns except age_of_birth to float
        cols_to_convert = [col for col in fighter1_stats.columns if col not in ('date_of_birth', 'fighter_name')]
        ...
        output_df['height_diff'] = fighter1_stats['height_cm'] - fighter2_stats['height_cm']
        output_df['age_diff'] = (pd.to_datetime(fighter1_stats['date_of_birth']) - pd.to_datetime(fighter2_stats['date_of_birth'])).dt.days / 365.25
        ...
        output_df['strike_acc_diff'] = fighter1_stats['strike_acc'] - fighter2_stats['strike_acc']
        ...
        output_df['strike_def_diff'] = fighter1_stats['strike_def'] - fighter2_stats['strike_def']
        output_df['sub_avg_diff'] = fighter1_stats['sub_avg'] - fighter2_stats['sub_avg']
        output_df['total_wins_diff'] = fighter1_stats['total_wins'] - fighter2_stats['total_wins']
        output_df['total_losses_diff'] = fighter1_stats['total_losses'] - fighter2_stats['total_losses']
        output_df['total_draws_diff'] = fighter1_stats['total_draws'] - fighter2_stats['total_draws']
        output_df['ufc_wins_diff'] = fighter1_stats['ufc_wins'] - fighter2_stats['ufc_wins']
        output_df['ufc_losses_diff'] = fighter1_stats['ufc_losses'] - fighter2_stats['ufc_losses']
