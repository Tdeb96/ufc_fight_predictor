...
class Bouts(scrapy.Spider):
    ...
    def parse_bouts_advanced(self, response, event_date):
        ...
        try: 
            ...
            bout_item['win'] = True
            bout_item['winner'] = response.xpath('//i[@class="b-fight-details__person-status b-fight-details__person-status_style_green"]/../div//a/text()').extract()[0].strip()

        except:
            bout_item['win'] = False
            bout_item['winner'] = '--'

        bout_item['fighter1'] = response.xpath('//div[@class="b-fight-details__persons clearfix"]//a/text()').extract()[0].strip()
        bout_item['fighter2'] = response.xpath('//div[@class="b-fight-details__persons clearfix"]//a/text()').extract()[1].strip()
